...
public class MessageHandlerContainer {
    private static final Logger LOG = LoggerFactory.getLogger(MessageHandlerContainer.class);
    @Autowired
    ...
    public void init() {
        ...
        LOG.info("[PostConstruct][消息处理器数量：{}]", handlers.size());
    }
    /**
    ...
    public static Class<? extends Message> getMessageClass(MessageHandler handler) {
        // 获得 Bean 对应的 最终Class 类。因为有可能被 AOP 代理过。
